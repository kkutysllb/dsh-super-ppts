...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var zh = {
			...
			tplChoose: "选择模板",
			tplFollow: "跟随默认模板",
			tplPickerSoon: "模板选择器即将开放（当前先跟随默认模板）",
			audienceLabel: "受众",
		...
		var en = {
			...
			tplPickerSoon: "Template picker is coming next (following the default for now)",
		 ...
		 * - Task 6：apply 注入的最小桥 createTask 换成完整 createTaskAndStart
		 *   （落盘 → 素材上传 → 会话桥 v3 提交 → 状态推进），本视图无需改动。 */
		...
		/**
		 * 任务创建最小桥（Task 3）。**Task 6 会用完整的 createTaskAndStart 替换它**
		 * ——届时 apply 里把 createTaskAndStart 作为 `createTask` 注入 makePanelsView，
		 * 视图层接口与调用形状（下方 input/output 契约）保持不变，视图无需改动。
		 *
		 * 本阶段只做「任务落盘」：api("tasks.create", {title, brief, workspace})，成功
		 * 后返回 { task, phase: "waiting-launch" } —— 如实反映「已落盘但尚未启动」，
		 * 不假装已经启动（自动启动依赖会话桥 v3 的 setDraft + submit）。
		 *   input  = { title, brief, workspace: {id,name,path}, materials }
		 *   output = Promise<{ task, phase: "started" | "waiting-launch", message? }>
		 * Task 6 在此基础上追加：素材上传（uploadMaterial）→ 会话桥 v3 提交 →
		 * 状态推进到 analyzing，并把 phase 置为 "started"。
		 */
		 ...
		 * options.bridges 是 apply 注入的桥（当前只有最小桥 createTask；Task 6 换成
		 * createTaskAndStart，调用形状与返回值语义保持不变）。
		 */
		function makeNewTaskView(t, options) {
			...
			function NewTaskView(props) {
				...
				// ── 模板（"" 跟随默认 / "none" 不使用 / 模板对象；Task 4 接真实选择器）──
				...
				var onFilesChosen = function (event) {
					...
					// Task 5：拿到 taskId 后经 bridges.uploadMaterial 真实上传，
					// 并以 materialStatus 把 pending 推进到 ready / failed。
				};
				...
				// ── 模板：Task 4 把这里换成真实选择器（makeTemplatePicker） ──
				...
				var tplRow = React.createElement("div", { className: "sp-field" },
					...
					React.createElement("button", {
						...
						title: t("tplChoose"),
						onClick: function () { setMsg(t("tplPickerSoon")); },
					}, tplLabel),
				);
			...
			try {
				...
				ctx.slots.inject("sidebar.panellist", function () {
					...
					var disposePanel = ctx.slots.register({
						name: "main",
						key: PANEL_ID,
					}, makePanelsView(t, { api: api, createTask: createTask }));
