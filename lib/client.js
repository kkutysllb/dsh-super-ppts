...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * 工作台主面板：工作区选择 + 新建 hero + 模板速览（与设置页职责分离）。
		 *
		 * 工作台 = 任务导向界面:工作区选择 + 主题一句话 + 交付形态 + 模板选择 →
		 * 创作提示词经草稿桥(v2:会话落点 → 输入框 setDraft;失败降级剪贴板)
		 * 进入当前会话,用户确认后回车发送;下方模板速览只读展示(管理在设置页)。
		 * 工作区行读宿主全局标准 hook useWorkspaces(root 面板自动注入;
		 * 旧宿主缺失时整行隐藏——与 panellist 同款软探测);默认「跟随当前
		 * 工作区」,跨工作区发起时由桥走 uiWorkspace.openWorkspace 复用/新建。
		 */
