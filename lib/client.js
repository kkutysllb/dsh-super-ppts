...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 *
		 * 视图容器类名（sp-view-new-task / sp-view-recent）由壳层随元素下发：
		 * 视图根沿用同一类名，宿主侧不产生多余包裹层。
		 */
		function makePanelsView(t, bridges) {
			function Panels(props) {
				...
				if (view === SP_VIEW_RECENT) {
					...
					}), Object.assign({ className: "sp-view-recent" }, viewProps));
				} else {
					body = React.createElement(makeNewTaskView(t, {
						onCreated: function () { refreshTasks(); setView(SP_VIEW_RECENT); },
						tasks: tasks,
					}), Object.assign({ className: "sp-view-new-task" }, viewProps));
				}
		 ...
		 * 根容器类名沿用壳层下发的 className（见 makePanelsView 注释），
		 * 单独渲染时回退到自身的默认类名。 */

		function makeNewTaskView(t, options) {
			function NewTaskView(props) {
				return React.createElement("div", {
					className: (props && props.className) || "sp-view-new-task",
				});
			}
			return NewTaskView;
		}
		...
		function makeRecentView(t, options) {
			function RecentView(props) {
				return React.createElement("div", {
					className: (props && props.className) || "sp-view-recent",
				});
			}
			return RecentView;
		}
