...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var zh = {
			...
			resultSummary: "已完成 · {pages} 页 · 可编辑 {format}",
			artifactReady: "可用",
		...
		var en = {
			...
			// ── 结果视图（Plan 2b Task 5）：与 zh 对齐的 14 键 ──
			resultSummary: "Completed · {pages} pages · editable {format}",
			artifactReady: "Ready",
		 ...
		 *   makeOutlineReviewView / makeProgressView / makeResultView；
		...
		function makePanelsView(t, bridges) {
			...
			function Panels(props) {
				...
				/** 离开详情回最近列表：同时刷新列表，保证轮询中变终态的任务回到列表时是最新状态。 */
				...
				// 活跃任务轮询：deps 只认 id+status——status 未变不重启循环（循环自身
				// 已按终态停），换任务/落点变化即重挂；effect 卸载（cancel）自动停。
				...
				if (view === SP_VIEW_RECENT) {
					...
					if (activeErr && !active) {
						...
						var detail = {
							task: active, api: apiFn,
							sendToSession: b.sendToSession,
							onUpdated: onTaskUpdated, onBack: backToRecent,
						};
						if (sub === "outline") {
							body = React.createElement(makeOutlineReviewView(t, detail), viewProps);
						} else if (sub === "progress") {
							detail.onBackToOutline = function () { setForcedSub("outline"); };
							// 视图契约是无参 openSession()；壳层桥接任务工作区 id
							//（apply 的 openSession(workspaceId) 先跳工作区再回会话）。
							detail.openSession = function () {
								if (typeof b.openSession === "function") {
									b.openSession(active.workspace && active.workspace.id);
								}
							};
							body = React.createElement(makeProgressView(t, detail), viewProps);
						} else if (sub === "result") {
							body = React.createElement(makeResultView(t, detail), viewProps);
						} else {
		...
		function makeOutlineReviewView(t, options) {
			...
			function OutlineReview() {
				...
				// dirty 的真实来源：渲染期直接对比 pages 与「已保存基线」。基线首渲染
				// 即 task 注入的 savedPages，tasks.outline / resync 成功后前移。
				var savedRef = React.useRef(savedPages);

				function dirtyNow() { return outlineDiffers(pages, savedRef.current); }
				...
				function saveOutlineEdits() {
					return apiFn("tasks.outline", { id: task.id, pages: pages }).then(function (saved) {
						savedRef.current = pages; // 刚落盘的内容成为新的 dirty 基线（host 会归一化）
						return saved;
					});
				}
				function saveOnly() {
					setBusy("save"); setError(""); setNotice("");
					return saveOutlineEdits().then(function () {
						...
						if (message.indexOf("版本不匹配") !== -1) {
							...
							apiFn("tasks.get", { id: task.id }).then(function (next) {
								if (next && next.outline && Array.isArray(next.outline.pages)) {
									...
									savedRef.current = next.outline.pages;
									setResync(true);
								}
							}).catch(function () {});
							return;
				...
				function deliverRevise(saveFirst) {
					...
					return Promise.resolve(prepared).then(function () {
						if (!saveFirst) { setPages(clonePages(savedRef.current)); }
				...
				function discardAndRevise() {
					setPages(clonePages(savedRef.current)); // 「丢弃修改」：回到已保存基线
					return deliverRevise(false);
				}
		 ...
		 * options: { task, api, sendToSession, onUpdated, onBack, onBackToOutline,
		 *   openSession, answerText?(仅测试) }
		 */
		function makeProgressView(t, options) {
			...
			var onUpdated = typeof opts.onUpdated === "function" ? opts.onUpdated : function () {};
			var onBack = typeof opts.onBack === "function" ? opts.onBack : function () {};
			var onBackToOutline = typeof opts.onBackToOutline === "function" ? opts.onBackToOutline : function () {};
			var openSession = typeof opts.openSession === "function" ? opts.openSession : function () {};
		 ...
		 * resultSummary 是 {pages}/{format} 占位模板，locale 未命中（t 原样回
		 * key）时回退内置 zh 模板——与 bundle 兜底 t 的 zh 优先语义一致。
		 ...
		 * + sp-msg-ok / sp-msg-err。
		 * options: { task, api, sendToSession, onUpdated, onBack, continueText?(仅测试) }
		 */
		function makeResultView(t, options) {
			...
			function Result() {
				...
				var summaryTpl = String(t("resultSummary"));
				var summaryText = fill(summaryTpl.indexOf("{pages}") !== -1 ? summaryTpl : String(zh.resultSummary), {
					pages: pageCount,
