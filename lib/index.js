 ...
 * 3. 原生工具注册：ppts_check（环境自检）与 ppts_render（渲染验收）。
 *
 * Cordis 契约（实装冒烟抓过的坑）：
 ...
 * - 无本地服务：不监听任何端口；PPTX 渲染验收走 CLI（tools.ts → compiler/
 *   与 skills/ppts-pptx/scripts/），HTML 产物是单文件直接浏览器打开；
...
import { pptsCheckTool, pptsRenderTool } from './tools.js';
    ...
    try {
        ...
        for (const name of ['preset.yml', 'agent.cordis.yml']) {
            ...
            if (existsSync(source))
                copyFileSync(source, resolve(userPresetDir, name));
        }
    }
    catch {
