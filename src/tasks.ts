 ...
 */
export function removeMaterial(id: string, materialId: string): TaskRecord {
  const task = requireTask(id)
  ...
  try { rmSync(material.path, { force: true }) } catch { /* 记录已一致，文件清理尽力而为 */ }
  return saved
}
...
export function setMaterialStatus(
  ...
  if (status === 'error') {
    ...
    if (text === '') delete material.error
    else material.error = text
  } else {
