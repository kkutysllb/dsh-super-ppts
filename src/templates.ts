...
function looksLikeRegistry(value: unknown): boolean {
...
function quarantineCorruptRegistry(): void {
...
function importLegacyStore(): void {
  if (LEGACY_STORE_ROOT === STORE_ROOT) return
  if (existsSync(REGISTRY_FILE)) return
  const legacyRegistry = join(LEGACY_STORE_ROOT, 'registry.json')
  if (!existsSync(legacyRegistry)) return
  try {
    ...
    if (existsSync(legacyTemplates)) {
      for (const name of readdirSync(legacyTemplates)) {
        ...
        if (!existsSync(target)) copyFileSync(join(legacyTemplates, name), target)
      }
    }
...
export function loadRegistry(): Registry {
  importLegacyStore()
  try {
    const parsed: unknown = JSON.parse(readRegistryText())
    if (!looksLikeRegistry(parsed)) {
      quarantineCorruptRegistry()
      return { templates: [], defaultTemplate: null, prefs: { ...PREFS_DEFAULTS } }
    }
    const shape = parsed as Partial<Registry>
    const templates = (Array.isArray(shape.templates) ? (shape.templates as TemplateRecord[]) : [])
      .map(item => reanchorRecord(item))
      .filter((item): item is TemplateRecord => item !== null)
    return {
      templates,
      defaultTemplate:
        typeof shape.defaultTemplate === 'string' && templates.some(item => item.id === shape.defaultTemplate)
    ...
    if (existsSync(REGISTRY_FILE)) quarantineCorruptRegistry()
    return { templates: [], defaultTemplate: null, prefs: { ...PREFS_DEFAULTS } }
  }
}
